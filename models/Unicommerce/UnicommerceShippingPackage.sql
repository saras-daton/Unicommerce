{% if var('UnicommerceShippingPackage') %}
    {{ config( enabled = True ) }}
{% else %}
...
{% endif %}
    ...
    {% set table_name_query %}
        {{set_table_name('%shippingpackage')}}    
    {% endset %} 
    ...
    {% for i in results_list %}
        ...
        From (
            ...
            DENSE_RANK() OVER (PARTITION BY ShippingPackageNo order by {{daton_batch_runtime()}} desc)row_num
            from {{i}} 
                {% if is_incremental() %}
